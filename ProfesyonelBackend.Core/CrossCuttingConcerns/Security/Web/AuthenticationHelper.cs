...
namespace ProfesyonelBackend.Core.CrossCuttingConcerns.Security.Web
{
    public class AuthenticationHelper
    {
        ...
        private static string CreateAuthTag(string email, string[] roles, string firstName, string lastName, Guid id)
        {
            ...
            stringBuilder.Append("|"); // Aralara eklenecek olan pipe
            for (int i = 0; i < roles.Length; i++)
            {
                stringBuilder.Append(roles[i]);//Rolleri ekleyecek
                if (i < roles.Length - 1)
                {
                    stringBuilder.Append(","); // Son gelen rol hariç her rol sonrası virgül ekleyecek
                }
            }
